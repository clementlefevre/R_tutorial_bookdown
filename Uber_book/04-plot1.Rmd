...
Here we just confirm what we saw in \@ref(aggregate_merge), it seems some _maxspeed_ are not "standard" or very seldomly used by Uber.
We can confirm this with a quick histogram :
```{r}
ggplot(DT.max.speed.per.way.hour,aes(maxspeed))+ geom_histogram()+theme_bw()
```
Indeed, 30,50,60,80 kph limits are our numbers.  
