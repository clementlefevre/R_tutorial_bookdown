...
Here we filter out the ways that are of class _motorway_ and _residential_:

```{r}
plot(st_geometry(OSM_sf%>% filter(fclass=='motorway')),col='red',reset=F)
plot(st_geometry(OSM_sf%>% filter(fclass=='residential')),col='blue',add=T)
```

What we did here is first plot the _motorway_ class, with the `reset` parameter set to `FALSE`, to allow the second plot call with the _residential_ class to drawn on the same element.
